...
export function StartScreen() {
  ...
  return (
    <div
      className={`pointer-events-auto absolute inset-0 z-40 overflow-y-auto backdrop-blur-sm max-lg:flex max-lg:items-center max-lg:justify-center max-lg:px-3 max-lg:pb-3 max-lg:pt-[calc(env(safe-area-inset-top)+3.25rem)] lg:grid lg:place-items-center lg:px-6 lg:pb-6 lg:pt-16 phone-land:!items-start phone-land:!justify-center phone-land:!overflow-y-auto phone-land:!px-2 phone-land:!pb-[max(0.35rem,env(safe-area-inset-bottom))] phone-land:!pt-[max(2.35rem,calc(env(safe-area-inset-top)+1.9rem))] ${
        isClubVenue ? 'bg-violet-950/85' : 'bg-neutral-950/80'
      }`}
    >
      <section
        className={`grid w-full max-w-4xl gap-6 rounded-[2rem] border p-5 shadow-2xl max-lg:max-h-[calc(100dvh-1rem)] max-lg:max-w-none max-lg:gap-3 max-lg:rounded-3xl max-lg:p-3 lg:grid-cols-[auto_1fr] lg:p-8 phone-land:!my-0 phone-land:!h-auto phone-land:!max-h-full phone-land:min-h-0 phone-land:grid-cols-[auto_1fr] phone-land:gap-2 phone-land:overflow-y-auto phone-land:p-2 ${
          isClubVenue
            ? 'border-fuchsia-400/40 bg-neutral-950 text-white'
            : 'border-white/15 bg-neutral-50 text-neutral-950'
        }`}
      >
        <div className="max-lg:hidden phone-land:block">
          {quest ? (
            <TraitQuestVisual
              traitType={quest.traitType}
              traitValue={quest.traitValue}
              sizeClassName="h-40 w-40 phone-land:!h-[4.5rem] phone-land:!w-[4.5rem]"
            />
          ) : (
            <TargetPreview
        ...
        <div className="relative phone-land:min-h-0">
          <p
          ...
          <h1 className="mt-3 text-3xl font-black tracking-tight max-lg:mt-1 max-lg:text-xl lg:text-5xl phone-land:mt-0 phone-land:text-lg">
            {t.title}
          </h1>
          <p
            className={`mt-4 text-base leading-relaxed max-lg:mt-1.5 max-lg:text-xs max-lg:leading-snug phone-land:hidden ${
          ...
          <div
            className={`mt-5 rounded-2xl border p-4 max-lg:mt-2 max-lg:p-2.5 phone-land:mt-1.5 phone-land:p-2 ${
              isClubVenue ? 'border-fuchsia-400/30 bg-violet-950/50' : 'border-neutral-200 bg-white'
            }`}
          >
            <p className={`text-sm font-semibold max-lg:text-xs phone-land:hidden ${isClubVenue ? 'text-fuchsia-200' : 'text-neutral-500'}`}>
              {t.gameMode}
            </p>
              ...
              {(['challenge', 'enjoy'] as const).map((mode) => {
                ...
                return (
                  <button
                    key={mode}
                    type="button"
                    className={`rounded-xl border px-3 py-2.5 text-left transition max-lg:px-2.5 max-lg:py-2 phone-land:px-2 phone-land:py-1.5 ${
                      isSelected
                        ? isClubVenue
                  ...
                  >
                    ...
                    <p
                      className={`mt-1 text-xs leading-snug max-lg:text-[0.65rem] phone-land:hidden ${
                        isSelected
                          ? isClubVenue
                            ? 'text-fuchsia-100'
                            : 'text-neutral-300'
          ...
          >
            <div className="flex items-center gap-3 phone-land:gap-2">
              {quest ? (
                <TraitQuestVisual
                  traitType={quest.traitType}
                  traitValue={quest.traitValue}
                  sizeClassName="h-20 w-20 shrink-0 lg:hidden phone-land:hidden"
                />
              ) : (
                <TargetPreview
                  capturePriority={START_SCREEN_TARGET_PREVIEW_PRIORITY}
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-3xl font-black max-lg:text-xl phone-land:text-lg">
                  {quest
                    ? t.findTraitLabel(
                        quest.findCount,
          ...
          <button
            type="button"
            className={`mt-6 w-full rounded-full px-6 py-3.5 text-sm font-black uppercase tracking-[0.25em] transition max-lg:mt-2.5 max-lg:py-3 max-lg:text-xs phone-land:mt-2 phone-land:py-2 ${
              isClubVenue
          ...
          {isClubVenue ? (
            ...
            <button
              type="button"
              className="after-hours-enter-pulse mt-3 w-full rounded-full border-2 border-violet-700 bg-gradient-to-r from-violet-700 via-fuchsia-600 to-violet-700 px-6 py-3.5 text-sm font-black uppercase tracking-[0.2em] text-white shadow-lg shadow-violet-600/40 transition hover:from-violet-600 hover:via-fuchsia-500 hover:to-violet-600 max-lg:py-3 max-lg:text-xs phone-land:mt-1.5 phone-land:py-2"
