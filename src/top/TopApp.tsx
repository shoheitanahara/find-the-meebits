...
function getAttractionPath(id: AttractionId) {
  ...
  if (id === 'opensea') return `${localePrefix}/opensea-market`
...
export function TopApp() {
  ...
  const [showParkTips, setShowParkTips] = useState(false)
  ...
  const requestEnterPark = () => {
    ...
    setShowParkTips(true)
    void unlockAudioIfNeeded().then(() => playSfx('uiClick'))
  }

  const confirmEnterPark = () => {
    if (!lineup) return
    setShowParkTips(false)
    const meebitNumber = normalizePlayerMeebitNumber(meebitInput)
  ...
  return (
    <main className="relative h-dvh w-dvw overflow-hidden bg-[#070914] text-[#f4ead2]">
      ...
      {showParkTips && parkReady ? <ParkTipsOverlay onConfirm={confirmEnterPark} /> : null}
